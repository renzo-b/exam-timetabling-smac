...
def minimize_mip(config, seed: int = 0):
    ...
    for instance_num in range(len(INSTANCE_SPACE)):
        ...
        objective_value = mip_solver.solve(
...
if __name__ == "__main__":
    ...
    configspace.add_hyperparameter(Categorical(
        "mip_covers_switch", [-1, 0, 1, 2, 3], default=0))
    configspace.add_hyperparameter(
        Float("number_of_cutting_plane_passes", [-1, 0], default=0))
    configspace.add_hyperparameter(Integer(
        "cut_factor_row_multiplier_limit", [-1, 10], default=-1))  # any postive float,
    configspace.add_hyperparameter(Categorical(
        "mip_dive_strategy", [0, 1, 2, 3], default=0))
    configspace.add_hyperparameter(Categorical(
        "dual_simplex_pricing_algorithm", [0, 1, 2, 3, 4, 5], default=0))
    configspace.add_hyperparameter(Categorical(
        "mip_subproblem_algorithm", [0, 1, 2, 3, 4, 5], default=0))
    configspace.add_hyperparameter(Categorical(
